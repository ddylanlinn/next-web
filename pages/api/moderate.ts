...
const approveComment = async (userID, id) => {
  const { error } = await supabase.from('comments').update({ published: true }).eq('id', id)
      ...
      blocks: [
        {
          ...
          text: {
            type: 'plain_text',
            text: `${type === 'approve_comment' ? 'Approvement' : 'Deletion'} succesful! ${
              type === 'approve_comment' ? ':white_check_mark:' : ':octagonal_sign:'
            }`,
            emoji: true,
          },
        },
